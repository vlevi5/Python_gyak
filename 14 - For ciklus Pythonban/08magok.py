...
for elem in megfelelo:
    total_magszam += elem[1] 
    total_ar += elem[2] 

if len(megfelelo) > 0:
    print(f"\nA processzorok magszámainak átlaga: {round(total_magszam/len(megfelelo))}")
    print(f"A processzorok átlagára: {round(total_ar/len(megfelelo))}")
else:
    print("A kritériumnak egy processzor sem felelt meg!")
